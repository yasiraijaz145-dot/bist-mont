      ...
      <main style={{ paddingTop: '110px', minHeight: '70vh' }}>
        <div style={{ background: '#f9f5f0', padding: '40px 0' }}>
          <div className="container">
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: '2.5rem', color: '#1a1a1a' }}>Your Cart</h1>
          </div>
        </div>
        ...
        <div className="container" style={{ paddingTop: '40px', paddingBottom: '80px' }}>
          {cart.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 0' }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display',serif", color: '#1a1a1a', marginBottom: '12px' }}>Your cart is empty</h2>
              <p style={{ color: '#888', marginBottom: '24px' }}>Add something delicious from our menu.</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '40px', alignItems: 'start' }}>
              ...
              <div>
                {cart.map((item, idx) => (
                  <div key={idx} style={{ display: 'grid', gridTemplateColumns: '80px 1fr auto', gap: '16px', alignItems: 'center', padding: '20px', background: '#fff', border: '1px solid #e8e0d5', borderRadius: '12px', marginBottom: '12px' }}>
                    ...
                    <img src={item.img} style={{ width: '80px', height: '80px', objectFit: 'cover', borderRadius: '8px' }}
                      ...
                      alt={item.name} />
                    <div>
                      <div style={{ fontWeight: 700, color: '#1a1a1a', marginBottom: '4px' }}>{item.name}</div>
                      <div style={{ color: '#e85d04', fontWeight: 700 }}>${item.price.toFixed(2)}</div>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#f9f5f0', borderRadius: '20px', padding: '4px 8px' }}>
                        <button onClick={() => changeQty(idx, -1)} style={{ width: '28px', height: '28px', borderRadius: '50%', border: '2px solid #e85d04', background: '#fff', color: '#e85d04', fontSize: '16px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
                        <span style={{ fontWeight: 700, minWidth: '20px', textAlign: 'center' }}>{item.qty}</span>
                        <button onClick={() => changeQty(idx, 1)} style={{ width: '28px', height: '28px', borderRadius: '50%', border: '2px solid #e85d04', background: '#fff', color: '#e85d04', fontSize: '16px', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>+</button>
                      </div>
                      <div style={{ fontWeight: 700, color: '#1a1a1a' }}>${(item.price * item.qty).toFixed(2)}</div>
                ...
                <Link href="/menu" style={{ display: 'inline-block', marginTop: '8px', color: '#e85d04', fontWeight: 600, textDecoration: 'underline' }}>+ Add more items</Link>
              </div>

              {/* Summary */}
              <div style={{ background: '#fff', border: '1px solid #e8e0d5', borderRadius: '12px', padding: '24px', position: 'sticky', top: '130px' }}>
                <h3 style={{ fontFamily: "'Playfair Display',serif", color: '#1a1a1a', marginBottom: '20px', paddingBottom: '16px', borderBottom: '1px solid #e8e0d5' }}>Order Summary</h3>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                  <input
                    ...
                    style={{ flex: 1, padding: '10px 12px', border: '2px solid #e8e0d5', borderRadius: '8px', fontSize: '14px', outline: 'none' }}
                  />
                  <button onClick={applyPromo} style={{ padding: '10px 16px', background: '#1a1a1a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer' }}>Apply</button>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '16px', borderTop: '2px solid #e8e0d5', fontSize: '18px', fontWeight: 800, color: '#1a1a1a' }}>
                  <span>Total</span><span style={{ color: '#e85d04' }}>${total.toFixed(2)}</span>
                </div>
                <Link href="/checkout" style={{ display: 'block', textAlign: 'center', marginTop: '20px', background: '#e85d04', color: '#fff', padding: '16px', borderRadius: '10px', fontWeight: 700, fontSize: '16px' }}>
                  Proceed to Checkout →
                </Link>
              </div>
            </div>
